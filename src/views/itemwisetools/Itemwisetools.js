...
// Function to compare data values for sorting
...
// Function to stable sort the data array
...
export default function Itemwisetool() {
    ...
    const fetchData = async () => {
      try {
        ...
        const formattedData = data.results.map((item, index) => ({
          ...
          clampmeter: item.clampmeter_date,
          plier: item.plier_date,
          hammer: item.hammer_date,
        }));
  ...
  const handlePdfDownload = () => {
    ...
    doc.autoTable({
      ...
      body: rows.map(row => [row.empcode, row.empname, row.clampmeter, row.plier, row.hammer]),
    });
