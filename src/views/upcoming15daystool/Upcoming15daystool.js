...
function EnhancedTableToolbar(props) {
  const { numSelected, handleDownloadPdf } = props;
...
export default function Upcoming15daystool() {
  ...
  const handleDownloadPdf = () => {
    const doc = new jsPDF();
    const tableColumn = ["Item Name", "Number"];
    const tableRows = [];
    ...
    doc.text("Tool List", 14, 15);
    doc.save("tool_list.pdf");
  };
  ...
  const sortedRows = useMemo(() => stableSort(rows, getComparator(order, orderBy)), [order, orderBy]);

  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar numSelected={selected.length} handleDownloadPdf={handleDownloadPdf} />
        <TableContainer>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right" onClick={() => handleNumberClick(row.id, row.protein)} style={{ cursor: 'pointer' }}>
                        {row.protein}
                      </TableCell>
                    </TableRow>
                  );
        ...
        <DialogContent>
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {selectedRow.map((employee, index) => (
                  <TableRow key={index}>
                    <TableCell>{employee.empCode}</TableCell>
                    <TableCell>{employee.empName}</TableCell>
                  </TableRow>
