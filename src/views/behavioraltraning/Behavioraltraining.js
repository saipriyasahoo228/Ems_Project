...
export default function Behavioraltraining() {
  ...
  const handlePdfDownload = () => {
    const doc = new jsPDF();
    doc.autoTable({
      head: [['SL NO.', 'BEHAVIORAL TRAINING', 'DATE', 'TRAINER NAME', 'REFERENCE NUMBER']],
      body: rows.map((row) => [row.slno, row.behavioraltraining, row.date, row.trainername, row.refrno]),
    });
